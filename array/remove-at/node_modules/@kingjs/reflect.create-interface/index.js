...
function hasInstance(instance) {
  ...
  for (var name of Object.getOwnPropertyNames(this)) {
    var symbol = this[name];
    if (symbol in prototype == false)
      return false;    
  }
  
  // test that inherited members are implemented
  for (var name in Object.getPrototypeOf(this)) {
    var symbolOrSymbols = this[name];

    // a name may be associated with many members
    if (is.array(symbolOrSymbols)) {
      for (var symbol in symbolOrSymbols) {
        if (symbol in prototype == false)
          return false;
      }
    }
